...
#define PRINT_INTERVAL 60          // time in seconds
...
void setup(void) {
  ...
  digitalWrite(RELAY_PIN, HIGH);    // Make sure relay is OFF
...
void loop(void) {
  ...
  int temp = 0;                     // Set it low so we won't turn on until we get a real temp
  int freezerState = 0;
...
int getTemperature(void) {
  ...
  return temp_int;    // return the rounded integer temperature
}
